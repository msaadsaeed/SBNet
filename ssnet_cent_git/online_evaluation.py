...
def eval_once(saver, placeholders, feat_img, feat_sent, img_test, img_test_voice, path):
...
def main(face_test, voice_test, path):
    im_feat_dim = 512
    sent_feat_dim = 512
    ...
    im_feat_plh = tf.placeholder(tf.float32, shape=[np.shape(face_test)[0], im_feat_dim])
    sent_feat_plh = tf.placeholder(tf.float32, shape=[np.shape(voice_test)[0], sent_feat_dim])
    ...
    eer, auc = eval_once(saver, placeholders, feat1, feat2, face_test, voice_test, path)
    # while True:
